...
OnInit.final("Boss", function(Require)
    ...
    local min, max, cos, sin, random = math.min, math.max, math.cos, math.sin, math.random
    ...
    do
        ...
        function Boss.create(index, loc_x, loc_y, facing, id, name, level, crystal, leash)
            ...
            -- bosses deal an additional 1 damage to hit based units
            Unit[self.unit].hit_damage = 2
        ...
        function thistype:switch_target(unit, delay)
            if unit then
                ...
                if size > 1 then -- atleast two units
                    local rand
                    repeat
                        rand = BlzGroupUnitAt(ug, random(0, size - 1))
