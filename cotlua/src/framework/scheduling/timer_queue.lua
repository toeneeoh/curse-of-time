...
do
    ...
    local function on_expire(timerQueue)
        ...
        if RuntimeMetrics then
            local metrics = RuntimeMetrics.timer_queue
            metrics.executed = metrics.executed + 1
    ...
    function TimerQueue:callDelayed(timeout, callback, ...)
        ...
        if RuntimeMetrics then
            local metrics = RuntimeMetrics.timer_queue
            metrics.scheduled = metrics.scheduled + 1
            metrics.active = metrics.active + 1
    ...
    function TimerQueue:reset()
        if RuntimeMetrics then
            RuntimeMetrics.timer_queue.active = RuntimeMetrics.timer_queue.active - self.n
        end
