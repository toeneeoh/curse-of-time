...
OnInit.final("Gluebutton", function(Require)
    ...
    local ATTACHMENT_MAX     = 3 ---@type integer
    local ATTACHMENT_WIDTH   = 0.165 ---@type number
    local ATTACHMENT_GAP     = 0.003 ---@type number
    local ATTACHMENT_ICON    = 0.024 ---@type number
    ...
    ---@field enableAttachments function
    ---@field attachment function
    ...
    do
        ...
        local attachment_context = 1000

        local function positionAttachments(self)
            if not self.attachments then
                return
            end

            local previous = self.box

            for i = 1, ATTACHMENT_MAX do
                local panel = self.attachments[i]
                BlzFrameClearAllPoints(panel.anchor)

                if i == 1 then
                    BlzFrameSetPoint(panel.anchor, FRAMEPOINT_TOPRIGHT, previous, FRAMEPOINT_TOPLEFT, -ATTACHMENT_GAP, 0.)
                else
                    BlzFrameSetPoint(panel.anchor, FRAMEPOINT_TOPRIGHT, previous, FRAMEPOINT_BOTTOMRIGHT, 0., -ATTACHMENT_GAP)
                end
        ...
        function thistype:visible(visibility)
            if visibility ~= nil then
                ...
                if self.attachments then
                    for i = 1, ATTACHMENT_MAX do
                        local panel = self.attachments[i]
                        BlzFrameSetVisible(panel.frame, visibility and panel.used)
                    end
                end
            end
        ...
        ---Creates three reusable compact panels for this tooltip.
        ---@return Tooltip
        function thistype:enableAttachments()
            if self.simple or self.attachments then
                return self
            end
            ...
            for i = 1, ATTACHMENT_MAX do
                local panel = {}
                attachment_context = attachment_context + 1
                panel.frame = BlzCreateFrame("TooltipBoxFrame", self.frame, 0, attachment_context)
                panel.box = BlzGetFrameByName("TooltipBox", attachment_context)
                panel.line = BlzGetFrameByName("TooltipSeperator", attachment_context)
                panel.icon = BlzGetFrameByName("TooltipIcon", attachment_context)
                panel.name = BlzGetFrameByName("TooltipName", attachment_context)
                panel.text = BlzGetFrameByName("TooltipText", attachment_context)
                panel.anchor = BlzCreateFrameByType("FRAME", "", self.frame, "", 0)
                panel.used = false

                BlzFrameSetSize(panel.anchor, 0.001, 0.001)
                BlzFrameSetEnable(panel.frame, false)
                BlzFrameSetSize(panel.icon, ATTACHMENT_ICON, ATTACHMENT_ICON)
                BlzFrameClearAllPoints(panel.icon)
                BlzFrameSetPoint(panel.icon, FRAMEPOINT_TOPLEFT, panel.anchor, FRAMEPOINT_TOPRIGHT,
                    -ATTACHMENT_WIDTH + 0.006, -0.006)
                BlzFrameSetEnable(panel.icon, false)

                BlzFrameClearAllPoints(panel.name)
                BlzFrameSetPoint(panel.name, FRAMEPOINT_LEFT, panel.icon, FRAMEPOINT_RIGHT, 0.006, 0.)
                BlzFrameSetFont(panel.name, "MasterFont", 0.010, 0)
                BlzFrameSetTextAlignment(panel.name, TEXT_JUSTIFY_MIDDLE, TEXT_JUSTIFY_LEFT)
                BlzFrameSetEnable(panel.name, false)

                BlzFrameClearAllPoints(panel.text)
                BlzFrameSetPoint(panel.text, FRAMEPOINT_TOPRIGHT, panel.anchor, FRAMEPOINT_TOPRIGHT, -0.006, -0.035)
                BlzFrameSetSize(panel.text, ATTACHMENT_WIDTH - 0.012, 0.)
                BlzFrameSetFont(panel.text, "MasterFont", 0.008, 0)
                BlzFrameSetTextAlignment(panel.text, TEXT_JUSTIFY_TOP, TEXT_JUSTIFY_LEFT)
                BlzFrameSetEnable(panel.text, false)

                BlzFrameClearAllPoints(panel.box)
                BlzFrameSetPoint(panel.box, FRAMEPOINT_TOPRIGHT, panel.anchor, FRAMEPOINT_TOPRIGHT, 0., 0.)
                BlzFrameSetPoint(panel.box, FRAMEPOINT_BOTTOMLEFT, panel.text, FRAMEPOINT_BOTTOMLEFT, -0.006, -0.006)

                BlzFrameSetVisible(panel.line, false)
                BlzFrameSetVisible(panel.frame, false)

                self.attachments[i] = panel
            end
        ...
        ---Updates or hides one compact attachment panel.
        ---@param index integer
        ---@param name string?
        ---@param description string?
        ---@param icon string?
        ---@return boolean
        function thistype:attachment(index, name, description, icon)
            local panel = self.attachments and self.attachments[index]
            if not panel then
                return false
            end
            ...
            if panel.used then
                BlzFrameSetText(panel.name, name)
                BlzFrameSetText(panel.text, description or "")
                if icon then
                    BlzFrameSetTexture(panel.icon, icon, 0, false)
                end
            end

            BlzFrameSetVisible(panel.frame, self.isVisible and panel.used)
