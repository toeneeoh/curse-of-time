...
OnInit.global("DevRuntimeLog", function(Require)
    ...
    function DevLog.snapshot(label)
        ...
        DevLog.write("METRICS", string.format(
            "%s init=%d/%d items=%d/%d/%d peak=%d events=%d callbacks=%d damage=%d ai=%d/%d timers=%d peak=%d mouse=%s ticks=%d sessions=%d active_s=%.3f avg_us=%.2f max_us=%.2f period=%.5f movespeed=%d peak=%d ticks=%d updates=%d corrections=%d recovery_ticks=%d sessions=%d active_s=%.3f avg_us=%.2f max_us=%.2f period=%.5f",
            label or "snapshot",
            ...
            movespeed.corrections, movespeed.recovery_ticks, movespeed.sessions,
            active_time(movespeed), average_sample_microseconds(movespeed),
