...
OnInit.final("StatView", function(Require)
    ...
    STAT_TAG[ITEM_DAMAGE].breakdown = function(u)
        return "|cffffcc00Base Damage:|r " .. BlzGetUnitBaseDamage(u, 0) ..
            ...
            "\n|cffffcc00Percent Bonus:|r " .. string.format("%.3f", (Unit[u].damage_percent - 1.) * 100.) .. "%" .. " (" .. string.format("%.3f", Unit[u].damage - Unit[u].bonus_damage - BlzGetUnitBaseDamage(u, 0)) .. ")" ..
    ...
    STAT_TAG[ITEM_REGENERATION].breakdown = function(u)
        return "|cffffcc00Flat Regeneration:|r " .. Unit[u].regen_flat ..
            "\n|cffffcc00Percent Regeneration:|r " .. string.format("%.3f", Unit[u].regen_max) .. "%" .. " (" .. string.format("%.3f", Unit[u].regen_max * Unit[u].hp * 0.01) .. ")" ..
            "\n|cffffcc00Healing Received:|r " .. string.format("%.3f", Unit[u].regen_percent * 100.) .. "%" ..
    ...
    STAT_TAG[ITEM_MANA_REGENERATION].breakdown = function(u)
        return "|cffffcc00Flat Regeneration:|r " .. Unit[u].mana_regen_flat ..
            ...
            "\n|cffffcc00Percent Regeneration:|r " .. string.format("%.2f", Unit[u].mana_regen_max) .. "%" .. " (" .. Unit[u].mana_regen_max * Unit[u].mana * 0.01 .. ")" ..
            "\n|cffffcc00Mana Received:|r " .. string.format("%.2f", Unit[u].mana_regen_percent * 100.) .. "%" ..
    ...
    STAT_TAG[ITEM_DAMAGE_RESIST].breakdown = function(u)
        ...
        local chaos = (chaos_reduc == 0.03 and "\n|cffffcc00Chaos Reduction:|r " .. string.format("%.3f", (1. - chaos_reduc) * 100) .. "%" or "")
        return "|cffffcc00Base Reduction:|r " .. string.format("%.3f", 100. - (HERO_STATS[GetType(u)].phys_resist) * 100.)  .. "%" ..
            "\n|cffffcc00Spell/Item Reduction:|r " .. string.format("%.3f", 100. - (Unit[u].dr * Unit[u].pr) / HERO_STATS[GetType(u)].phys_resist * 100.)  .. "%" ..
            "\n|cffffcc00Armor Reduction:|r " .. string.format("%.3f", ((0.05 * BlzGetUnitArmor(u)) / (1. + 0.05 * BlzGetUnitArmor(u))) * 100.)  .. "%" ..
            chaos ..
            "\n|cffffcc00Total Reduction:|r " .. string.format("%.3f", 100. - (Unit[u].dr * Unit[u].pr) * 100. * (1. - ((0.05 * BlzGetUnitArmor(u)) / (1. + 0.05 * BlzGetUnitArmor(u)))) * chaos_reduc) .. "%"
    end

    STAT_TAG[ITEM_DAMAGE_RESIST].getter = function(u)
        local dtype = BlzGetUnitIntegerField(u, UNIT_IF_DEFENSE_TYPE)
        local chaos_reduc = (dtype == ARMOR_CHAOS or dtype == ARMOR_CHAOS_BOSS) and 0.03 or 1.
        return string.format("%.3f", (Unit[u].dr * Unit[u].pr) * 100. * (1. - ((0.05 * BlzGetUnitArmor(u)) / (1. + 0.05 * BlzGetUnitArmor(u)))) * chaos_reduc)
    end
    ...
    STAT_TAG[ITEM_MAGIC_RESIST].breakdown = function(u)
        ...
        local chaos_reduc = (dtype == ARMOR_CHAOS or dtype == ARMOR_CHAOS_BOSS) and 0.03 or 1.
        local chaos = (chaos_reduc == 0.03 and "\n|cffffcc00Chaos Reduction:|r " .. string.format("%.3f", (1. - chaos_reduc) * 100) .. "%" or "")
        return "|cffffcc00Base Reduction:|r " .. string.format("%.3f", 100. - (HERO_STATS[GetType(u)].magic_resist) * 100.)  .. "%" ..
            "\n|cffffcc00Spell/Item Reduction:|r " .. string.format("%.3f", 100. - (Unit[u].dr * Unit[u].mr) / HERO_STATS[GetType(u)].magic_resist * 100.)  .. "%" ..
            chaos ..
            "\n|cffffcc00Total Reduction:|r " .. string.format("%.3f", 100. - (Unit[u].dr * Unit[u].mr) * 100. * chaos_reduc) .. "%"
    end

    STAT_TAG[ITEM_MAGIC_RESIST].getter = function(u)
        local dtype = BlzGetUnitIntegerField(u, UNIT_IF_DEFENSE_TYPE)
        local chaos_reduc = (dtype == ARMOR_CHAOS or dtype == ARMOR_CHAOS_BOSS) and 0.03 or 1.
        return string.format("%.3f", (Unit[u].dr * Unit[u].mr) * 100. * chaos_reduc)
    end

    STAT_TAG[ITEM_DAMAGE_MULT].getter = function(u) return string.format("%.3f", (Unit[u].dm * Unit[u].pm) * 100.) end
    STAT_TAG[ITEM_MAGIC_MULT].getter = function(u) return string.format("%.3f", (Unit[u].dm * Unit[u].mm) * 100.) end
    ...
    STAT_TAG[ITEM_SPELLBOOST].getter = function(u) return string.format("%.3f", Unit[u].spellboost * 100.) end
    STAT_TAG[ITEM_CRIT_CHANCE].getter = function(u) return string.format("%.2f", Unit[u].cc) end
    STAT_TAG[ITEM_CRIT_DAMAGE].getter = function(u) return string.format("%.2f", Unit[u].cd) end
    STAT_TAG[ITEM_CRIT_CHANCE_MULT].getter = function(u) return string.format("%.2f", Unit[u].cc) end
    STAT_TAG[ITEM_CRIT_DAMAGE_MULT].getter = function(u) return string.format("%.2f", Unit[u].cd * 100.) end
    STAT_TAG[ITEM_BASE_ATTACK_SPEED].getter = function(u) local as = BlzGetUnitWeaponBooleanField(u, UNIT_WEAPON_BF_ATTACKS_ENABLED, 0) and 1. / Unit[u].bat or 0 return string.format("%.2f", as) .. " attacks per second" end
    STAT_TAG[ITEM_GOLD_GAIN].getter = function(u) local pid = GetPlayerId(GetOwningPlayer(u)) + 1 return Unit[u].gold_rate end
    STAT_TAG[ITEM_STACK + 1].getter = function(u) local as = BlzGetUnitWeaponBooleanField(u, UNIT_WEAPON_BF_ATTACKS_ENABLED, 0) and (1. / Unit[u].bat) * (1 + math.min(GetHeroAgi(u, true), 400) * 0.01) or 0 return string.format("%.2f", as) .. " attacks per second" end
    STAT_TAG[ITEM_STACK + 2].getter = function(u) local pid = GetPlayerId(GetOwningPlayer(u)) + 1 return string.format("%.2f", Unit[u].xp_rate) end
